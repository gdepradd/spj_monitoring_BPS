        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="border-l-2 border-gray-200 ml-4 space-y-6">
                    @foreach($timeline as $t)
                        <div class="relative pl-6">
                            ...
                            <p class="text-sm font-semibold text-gray-900">{{ $t['judul'] }} oleh {{ $t['aktor'] ?? 'Sistem' }}</p>
                            <p class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($t['waktu'])->format('d M Y H:i') }}</p>
                            <p class="text-sm text-gray-700 mt-1">Status: <strong>{{ $t['nama_status'] }}</strong></p>
                            @if(!empty($t['catatan']))
                                <p class="text-sm text-gray-600 bg-gray-50 p-2 rounded mt-1">Catatan: {{ $t['catatan'] }}</p>
                            @endif
                        </div>
                    @endforeach
                </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6" x-data="{ keputusan: '' }">
                ...
                <form action="{{ route('verifikator.pengajuan.keputusan', $pengajuan->id_pengajuan) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="mt-2 space-y-2">
                            <label class="inline-flex items-center">
                                <input type="radio" name="id_status_verifikasi" value="1" x-model="keputusan" class="text-green-600" required>
                                <span class="ml-2">ACC</span>
                            </label>
                            <label class="inline-flex items-center ml-4">
                                <input type="radio" name="id_status_verifikasi" value="2" x-model="keputusan" class="text-orange-600" required>
                                <span class="ml-2">REVISI</span>
                            </label>
                            <label class="inline-flex items-center ml-4">
                                <input type="radio" name="id_status_verifikasi" value="3" x-model="keputusan" class="text-red-600" required>
                    ...
                    <div class="mb-4">
                        <label for="catatan" class="block font-medium text-sm text-gray-700">Catatan <span x-show="keputusan == '2' || keputusan == '3'" class="text-red-500">*</span></label>
                        <textarea name="catatan" id="catatan" rows="3" class="w-full border-gray-300 rounded-md shadow-sm" :required="keputusan == '2' || keputusan == '3'"></textarea>
